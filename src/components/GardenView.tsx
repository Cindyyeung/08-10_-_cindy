...
import { PlantState, CheckInRecord, FoodInventory, FoodKey, FOOD_ITEMS_LIST, ToyInventory, ToyKey, TOY_ITEMS_LIST } from '../types';
import { playClickSound, playSuccessChime, speakText } from '../utils/audio';
import { ArrowLeft, Gift, Copy, Check, Home, Edit3, Download, Share2, Sparkles, RefreshCw, Volume2, BookOpen, Library, CheckCircle2, X, Camera, ChevronDown, ChevronUp } from 'lucide-react';
import { ALL_CARDS } from '../cardsData';
import { useLanguage } from '../i18n/LanguageContext';
import { getTranslatedPlantStage } from '../moodsData';
...
const POSTCARD_THEMES = [
  {
    ...
    illustration: '☀️🌸🌿',
    circleStyle: 'bg-gradient-to-tr from-amber-300 via-orange-300 to-rose-300 border-amber-400'
  },
  {
    name: '薄荷森林',
    ...
    illustration: '🌱🌳☘️',
    circleStyle: 'bg-gradient-to-tr from-teal-300 via-emerald-300 to-green-300 border-emerald-400'
  },
  {
    name: '薰衣之境',
    ...
    illustration: '🌙✨🌷',
    circleStyle: 'bg-gradient-to-tr from-indigo-300 via-purple-300 to-pink-300 border-purple-400'
  },
  {
    name: '蔚藍海岸',
    ...
    illustration: '🌊🕊️🍃',
    circleStyle: 'bg-gradient-to-tr from-blue-300 via-cyan-300 to-teal-300 border-cyan-400'
  }
];
...
export default function GardenView({
  ...
  const [postcardName, setPostcardName] = useState(language === 'en' ? 'Friend' : '同學');
  ...
  const getStageInfo = (count: number): { stageName: string; label: string; desc: string; progress: number; stageIndex: 1 | 2 | 3 | 4 | 5 | 6 } => {
    if (count <= 1) { // 0-1/10
      return {
        stageName: language === 'en' ? 'Seed Stage' : '種子期',
        label: language === 'en' ? 'Seed Stage (0-1/10)' : '種子期（0-1/10）',
        desc: language === 'en' ? 'The seed is sleeping in the soil, waiting to sprout 🌰' : '種子沉睡在土壤裏面，等待發芽 🌰',
        progress: Math.round((count / 10) * 100),
        stageIndex: 1,
      };
    } else if (count <= 3) { // 2-3/10
      return {
        stageName: language === 'en' ? 'Sprout Stage' : '發芽期',
        label: language === 'en' ? 'Sprout Stage (2-3/10)' : '發芽期（2-3/10）',
        desc: language === 'en' ? 'The tiny sprout peeks out, curious about the world 👀' : '小苗剛探出頭來，對世界充滿好奇 👀',
        progress: Math.round((count / 10) * 100),
        stageIndex: 2,
      };
    } else if (count === 4) { // 4/10
      return {
        stageName: language === 'en' ? 'Seedling Stage' : '幼苗期',
        label: language === 'en' ? 'Seedling Stage (4/10)' : '幼苗期（4/10）',
        desc: language === 'en' ? 'Growing up gently with new progress every day 🌿' : '正在溫柔地長高，每一天都有新的變化 🌿',
        progress: 40,
        stageIndex: 3,
      };
    } else if (count <= 6) { // 5-6/10
      return {
        stageName: language === 'en' ? 'Bud Stage' : '花蕾期',
        label: language === 'en' ? 'Bud Stage (5-6/10)' : '花蕾期（5-6/10）',
        desc: language === 'en' ? 'Leaves grow lush and full of vibrant life 🌿🌿' : '葉子漸漸變得茂密，充滿了生命力 🌿🌿',
        progress: Math.round((count / 10) * 100),
        stageIndex: 4,
      };
    } else if (count <= 8) { // 7-8/10
      return {
        stageName: language === 'en' ? 'Budding Stage' : '含苞期',
        label: language === 'en' ? 'Budding Stage (7-8/10)' : '含苞期（7-8/10）',
        desc: language === 'en' ? 'Waiting quietly, a beautiful bloom is about to begin 🌸' : '靜靜等待，一場美好的盛放即將開始🌸',
        progress: Math.round((count / 10) * 100),
        stageIndex: 5,
      };
    } else { // 9-10/10 (盛開期)
      return {
        stageName: language === 'en' ? 'Bloom Stage' : '盛開期',
        label: language === 'en' ? 'Bloom Stage (9-10/10)' : '盛開期（9-10/10）',
        desc: language === 'en' ? 'All your thoughtful care blooms brilliantly right now 💐' : '所有的細心陪伴，都在這一刻盛開了💐',
  ...
  const handleHarvest = () => {
    ...
    setToastMessage('✅ 收割成果 +10\n🎉 收割成功！花園已重獲新生，重新播下希望的種子！🌱');
    setTimeout(() => {
        ...
        {isQuoteCard ? (
          ...
          <div className="flex-1 flex flex-col justify-center items-center py-2 px-1 space-y-4 max-w-[360px] mx-auto w-full my-auto">
            ...
            >
              ...
              <div className="space-y-1 pt-1">
                ...
                <p className="text-[14px] sm:text-[15px] font-bold text-gray-400 font-mono">
                  {new Date().toISOString().split('T')[0]}
                </p>
                <p className="text-[16px] sm:text-[17px] font-black text-gray-800 pt-1">
                  你已成功兌換「實體語錄卡牌」1 張 🃏
                </p>
              </div>
              ...
              <div className="bg-amber-50/80 border-2 border-dashed border-amber-300/80 rounded-2xl p-3.5 text-left space-y-1">
                <p className="text-[15px] sm:text-[16px] font-black text-amber-900 flex items-center gap-1">
                  <span>📌</span>
                  <span>領取方式：</span>
                </p>
                <p className="text-[14px] sm:text-[15px] font-extrabold text-gray-700 pl-5 leading-relaxed">
                  請向老師出示此畫面，即可領取
                </p>
              </div>
              ...
              <div className="space-y-2 pt-1 border-t border-gray-100">
                <div className="text-[13px] font-black text-gray-400 tracking-widest uppercase">
                  ─── 下一步 ───
                </div>
                <div className="space-y-0.5">
                  <p className="text-[15px] sm:text-[16px] font-black text-gray-800 flex items-center justify-center gap-1">
                    <span>📷</span>
                    <span>掃描語錄卡</span>
                  </p>
                  <p className="text-[13px] sm:text-[14px] font-bold text-gray-500">
                    拿到實體卡後，掃描卡上文字入圖鑑 ✨
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2.5 pt-2">
                <button
                  ...
                  className="py-2.5 px-3 bg-brand-sage hover:bg-brand-moss text-white rounded-xl text-[14px] sm:text-[16px] font-black transition cursor-pointer flex items-center justify-center gap-1.5 active:scale-95 shadow-2xs border-0"
                ...
                <button
                  ...
                  className="py-2.5 px-3 bg-[#f9f7f2] hover:bg-brand-sand text-brand-moss border border-brand-sand/80 rounded-xl text-[14px] sm:text-[16px] font-black transition cursor-pointer flex items-center justify-center gap-1.5 active:scale-95 shadow-2xs"
                  style={{ minHeight: '42px' }}
                >
          ...
          <div className="flex-1 flex flex-col justify-between space-y-4 py-2">
            ...
            <div className="relative shrink-0 py-1">
              <motion.div
                layout
                className={`relative w-full ${isIpad ? 'max-w-[560px] p-6' : 'max-w-[440px] p-4'} mx-auto rounded-2xl shadow-md border-2 overflow-hidden transition-all duration-300 bg-gradient-to-br ${POSTCARD_THEMES[postcardTheme].gradient} ${POSTCARD_THEMES[postcardTheme].border} ${POSTCARD_THEMES[postcardTheme].text}`}
              >
                ...
                {/* Postcard Layout */}
                <div className={`flex flex-col h-full justify-between ${isIpad ? 'min-h-[280px]' : 'min-h-[220px]'}`}>
                  <div className="grid grid-cols-[1.2fr_1px_1fr] gap-3 relative h-full">
                    
            {/* Left Side: Quote & Polaroid-framed Plant Snapshot */}
            <div className={`pr-1 flex flex-col justify-between h-full ${isIpad ? 'min-h-[280px]' : 'min-h-[220px]'}`}>
              {/* Top: Quotes (上面是語錄) */}
              <div className="text-left select-none mb-1">
                <p className={`${isIpad ? 'text-[15px]' : 'text-xs sm:text-[13px]'} font-black leading-relaxed italic text-slate-700 font-sans`}>
                  「{postcardQuote}」
                </p>
              </div>
              ...
              <div className="flex-1 flex items-center justify-center relative mt-1 select-none">
                <div className={`bg-white p-2 pb-4 rounded shadow-[0_4px_12px_rgba(0,0,0,0.1)] border border-slate-100 ${isIpad ? 'w-36' : 'w-28 sm:w-32'} rotate-[-1.5deg] transform hover:rotate-0 hover:scale-105 transition-all duration-300`}>
                  <div className="aspect-square w-full bg-linear-to-b from-slate-50 to-slate-100 border border-slate-200/50 overflow-hidden relative flex items-center justify-center rounded-xs">
                    ...
                    <div className={`${isIpad ? 'scale-[0.58]' : 'scale-[0.45] sm:scale-[0.48]'} origin-center w-40 h-40 flex items-center justify-center select-none pointer-events-none`}>
                      <AnimatedPlant
                  ...
                  <div className="mt-2 text-center">
                    <span className={`font-serif font-black ${isIpad ? 'text-[13px] max-w-[130px]' : 'text-[10px] sm:text-[11px] max-w-[100px]'} text-slate-700 italic block tracking-wider truncate mx-auto`}>
                      🌱 {plantState.name}
                    </span>
                  </div>
                </div>
                    ...
                    <div className={`pl-2 flex flex-col justify-between relative h-full ${isIpad ? 'min-h-[280px]' : 'min-h-[220px]'} pb-1`}>
                      ...
                      <div className="absolute top-0 right-0 flex flex-col items-center select-none">
                        ...
                        <div className={`${isIpad ? 'w-13 h-17 p-1.5' : 'w-11 h-14 p-1'} border border-dashed rounded flex flex-col items-center justify-between relative shadow-xs ${POSTCARD_THEMES[postcardTheme].stampBg} border-slate-600/40`}>
                          <div className={`${isIpad ? 'text-[8px]' : 'text-[7px]'} font-bold tracking-widest text-slate-500 leading-none`}>POSTAGE</div>
                          <div className={`${isIpad ? 'text-lg' : 'text-sm'} leading-none my-0.5`}>🌸</div>
                          <div className={`${isIpad ? 'text-[9px]' : 'text-[8px]'} font-black text-slate-500 leading-none`}>$2.40</div>
                        </div>
                        {/* Postmark Circle overlay */}
                        <div className={`absolute -top-1.5 -right-1.5 ${isIpad ? 'w-16 h-16' : 'w-14 h-14'} rounded-full border border-dashed border-slate-500/25 flex items-center justify-center rotate-12 pointer-events-none`}>
                          <span className={`${isIpad ? 'text-[8px]' : 'text-[7px]'} font-bold text-slate-400/55 font-mono tracking-widest scale-90 whitespace-nowrap`}>
                            HONG KONG
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="pt-16 space-y-2 text-left flex-1 flex flex-col justify-end">
                        <div className="space-y-0.5">
                          <span className={`${isIpad ? 'text-[11px]' : 'text-[9px]'} font-bold tracking-wider text-slate-400 block`}>收件人 / To:</span>
                          <div className="border-b border-slate-500/30 pb-0.5 pl-1">
                            <span className={`${isIpad ? 'text-[16px] max-w-[180px]' : 'text-[13px] max-w-[130px]'} font-bold tracking-wide font-sans text-slate-800 truncate block`}>
                              {postcardName || '親愛的同學'}
                            </span>
                          </div>
                        </div>
            ...
            <div className={`bg-white/80 p-3.5 rounded-2xl border border-brand-sand/50 space-y-3.5 w-full ${isIpad ? 'max-w-[560px]' : 'max-w-[440px]'} mx-auto text-left`}>
              {/* Sign Name input */}
              <div className="space-y-1">
                <label className={`${isIpad ? 'text-sm' : 'text-xs'} font-black text-brand-moss flex items-center gap-1.5`}>
                ...
                <input
                  ...
                  className={`w-full px-3 py-1.5 border border-brand-sand rounded-xl ${isIpad ? 'text-sm' : 'text-xs'} focus:outline-none focus:border-brand-sage font-black text-gray-700 bg-white shadow-2xs`}
                  placeholder="輸入你的名字..."
                />
              </div>

              {/* Theme selection row with Color Circles instead of text */}
              <div className="space-y-1.5">
                <div className="flex items-center justify-between">
                  <span className={`${isIpad ? 'text-sm' : 'text-xs'} font-black text-brand-moss flex items-center gap-1.5`}>
                    <span>🎨</span> <span>選擇明信片款式（即時生成）：</span>
                  </span>
                  <div className={`flex items-center ${isIpad ? 'gap-2.5' : 'gap-1.5'}`}>
                    {POSTCARD_THEMES.map((theme, idx) => (
                      <button
                        ...
                        className={`${isIpad ? 'w-7 h-7' : 'w-5.5 h-5.5'} rounded-full border-2 transition-all cursor-pointer ${theme.circleStyle} ${
                          postcardTheme === idx
                            ? 'ring-2 ring-offset-2 ring-brand-moss scale-110 shadow-sm'
          ...
          <div className="flex-1 flex flex-col justify-between py-1 px-1 h-full overflow-y-auto">
            <div className="flex-1 flex flex-col items-center justify-center py-6 text-center space-y-6">
              ...
              <div className="bg-white/80 p-5 rounded-2xl border border-brand-sand/60 w-full max-w-[340px] shadow-xs space-y-4 font-sans text-left">
                <div>
                  <span className="text-[14px] sm:text-[15px] text-gray-400 font-bold block mb-1">你已成功兌換：</span>
                  <p className="text-[18px] sm:text-[19px] font-black text-gray-800">「{exchangedGift.name}」</p>
                </div>

                <div className="bg-brand-sand/20 px-3 py-2.5 rounded-xl border border-brand-sand/50 font-mono">
                  <span className="text-[13px] sm:text-[14px] text-gray-400 font-bold block">兌換碼：</span>
                  <p className="text-[17px] sm:text-[18px] font-black text-brand-moss tracking-wider mt-0.5">{successCode}</p>
                </div>

                <div className="space-y-1 pt-1.5 border-t border-brand-sand/40 text-[14px] sm:text-[15px] font-bold text-gray-600">
  ...
  // Render Page: GARDEN MANAGEMENT SCREEN
  if (mode === 'management') {
    const comps = plantState.companions || {};
    ...
    const potTheme = plantState.potTheme || 'default';
    const activeDecorations = plantState.activeDecorations || [];

    const pots = [
      { id: 'default', name: '經典紅陶', emoji: '🪴', unlocked: true },
      { id: 'rainbow', name: '彩虹漸層花盆', emoji: '🌈', unlocked: unlockedPrizes.includes(3) || potTheme === 'rainbow' },
      { id: 'cloud', name: '飄飄白雲花盆', emoji: '☁️', unlocked: unlockedPrizes.includes(5) || potTheme === 'cloud' },
    ];

    const decorations = [
      { id: 4, name: '簡約石板步道', emoji: '🧱', unlocked: unlockedPrizes.includes(4) || activeDecorations.includes(4) },
      { id: 6, name: '精靈小屋', emoji: '🧚‍♀️', unlocked: unlockedPrizes.includes(6) || activeDecorations.includes(6) },
      { id: 7, name: '許願噴泉', emoji: '⛲', unlocked: unlockedPrizes.includes(7) || activeDecorations.includes(7) },
      { id: 8, name: '暖心太陽', emoji: '☀️', unlocked: unlockedPrizes.includes(8) || activeDecorations.includes(8) },
    ];
                  ...
                  >
                    ...
                    {d.unlocked ? (
                      <button
                        ...
                        onClick={() => {
                          playClickSound(400, 'sine');
                          const updated = isPlaced
                            ? activeDecorations.filter((id) => id !== d.id)
    ...
    return (
      <div className="flex-1 flex flex-col justify-between py-1 px-1 h-full overflow-y-auto">
        <div className="space-y-4">
          ...
          {/* Centered Neat Score Counter Card & Garden Management Entrance */}
          <div className="flex items-center justify-between max-w-[320px] mx-auto w-full gap-2">
            <div className="bg-[#f9f7f2] p-2.5 px-4 rounded-xl border border-brand-sand/80 shadow-xs flex items-center justify-between flex-1">
            ...
            <button
              ...
              className="bg-amber-500 text-white border border-amber-600 px-3.5 py-2 rounded-xl text-xs font-black flex items-center gap-1 hover:bg-amber-600 transition active:scale-95 cursor-pointer shadow-xs whitespace-nowrap"
            >
              <span>🛠️ 花園管理</span>
            </button>
          </div>
          ...
          <div className="space-y-3.5 pb-8">
            ...
            {/* 🎒 精靈背包 (Sprite Backpack Overview) moved to bottom */}
            <div className="pt-4 pb-2">
              <div 
                className="mt-2 mb-2 p-3.5 bg-brand-sand/30 hover:bg-brand-sand/50 rounded-2xl border-2 border-brand-sand/60 cursor-pointer select-none transition-all flex items-center justify-between group shadow-sm active:scale-[0.98]"
                onClick={() => {
                  playClickSound(500, 'sine');
              ...
              >
                <div className="flex items-center gap-3.5">
                  ...
                  <div className="flex flex-col items-start gap-0.5">
                    <span className="text-sm font-black text-brand-moss tracking-wider">精靈背包</span>
                    <span className="text-[11px] font-bold text-gray-500">記錄心情即可累積儲存</span>
                  </div>
                </div>
                <div className="w-9 h-9 rounded-full bg-white flex items-center justify-center shadow-xs border border-brand-sand/80 group-hover:border-brand-sage transition-colors">
                  {isBackpackOpen ? (
                    <ChevronUp className="w-5 h-5 text-brand-sage" />
                  ) : (
            ...
            <AnimatePresence>
              {isBackpackOpen && (
                ...
                >
                  <div className={isIpad ? "grid grid-cols-2 gap-3.5 pb-8 pt-2" : "grid grid-cols-1 gap-3.5 pb-8 pt-2"}>
                    {/* Render Foods */}
                    {FOOD_ITEMS_LIST.map((item) => {
                      const count = foodInventory ? (foodInventory[item.id] || 0) : 0;
                      return (
                        <div
                          key={item.id}
                          className="bg-white p-4 rounded-2xl border border-brand-sand flex items-center justify-between gap-4 shadow-xs hover:border-amber-200/60 transition-all duration-300"
                        >
                          <div className="space-y-1 text-left flex-1">
                            <h3 className="text-sm font-black text-gray-800 flex items-center gap-1">
                          ...
                          <div className="shrink-0 flex items-center justify-center">
                            <span className="text-xs font-black text-amber-700 bg-amber-50 px-3 py-1.5 rounded-full border border-amber-200/60 shadow-xs">
                              {count} 件
                            </span>
                          </div>
                        </div>
                    ...
                    {/* Render Toys */}
                    {TOY_ITEMS_LIST.map((item) => {
                      const count = toyInventory ? (toyInventory[item.id] || 0) : 0;
                      return (
                        <div
                          key={item.id}
                          className="bg-white p-4 rounded-2xl border border-brand-sand flex items-center justify-between gap-4 shadow-xs hover:border-amber-200/60 transition-all duration-300"
  ...
  return (
    <div className="flex-1 flex flex-col justify-between py-1 px-1 space-y-2 h-full overflow-y-auto overflow-x-hidden relative">
      {/* Centered Garden Title & Growth Showcase Button */}
      <div className="text-center relative shrink-0 space-y-1 mt-2">
        <h2 className="text-[20px] sm:text-[22px] font-black text-gray-800 font-sans tracking-tight">
          {language === 'en' ? 'My Garden' : '我的花園'}
        </h2>
        <button
          type="button"
          onClick={() => {
          ...
          title={language === 'en' ? 'Watch 6-stage growth animation' : '點擊觀賞盆栽與花卉的 6 階段生長動畫'}
        >
          <Sparkles className="w-3.5 h-3.5 text-amber-600 animate-pulse" />
          <span>{language === 'en' ? '🌱 Watch 6-Stage Growth Animation' : '🌱 觀賞 6 階段花卉生長動畫賞'}</span>
        </button>
      </div>
      ...
      <div className={`flex flex-col items-center justify-center relative py-0 shrink-0 ${isIpad ? 'min-h-[260px]' : 'min-h-[200px]'}`}>
        <AnimatedPlant
          key={plantState.theme}
          progress={stageInfo.progress}
          stageIndex={stageInfo.stageIndex}
        ...
        {!isCatAdopted ? (
          <CatAdoptionOrb
            key={`cat-orb-garden-${isIpad ? 'ipad' : 'phone'}`}
            onAdoptComplete={onAdoptComplete || (() => {})}
            isIpad={isIpad}
          />
        ) : (
          <AnimatedCat
            key={`cat-garden-${isIpad ? 'ipad' : 'phone'}`}
      ...
      <div className="text-center space-y-1.5 shrink-0 px-2 mt-0 relative z-10">
        {/* Plant Name editor */}
          ...
          {isEditingName ? (
            ...
            <div className="flex items-center gap-1.5">
              ...
              <button
                ...
                title={language === 'en' ? 'Rename plant' : '修改盆栽名字'}
              >
        ...
        <div className="space-y-1.5 max-w-sm mx-auto w-full pt-1">
          <div className="flex items-center justify-between text-xs sm:text-sm font-black text-brand-moss font-sans px-0.5">
            <span>
              {language === 'en' 
                ? `Progress (${displayCount}/10) · ${stageInfo.stageName}`
                : `進度條 （${displayCount}/10） · ${stageInfo.stageName}`
              }
            </span>
      ...
      <div className="bg-[#f9f7f2] p-2.5 px-4 rounded-xl border border-brand-sand/80 shadow-xs flex items-center justify-between shrink-0 max-w-[280px] mx-auto w-full">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[13px] font-extrabold text-brand-moss font-sans">
            {language === 'en' ? 'My Points:' : '我的心晴積分：'}
          </span>
        </div>
        <span className="text-[14px] font-black text-amber-600 font-mono">
          {score} {language === 'en' ? 'pts' : '分'}
        </span>
      </div>
      ...
      <div className="space-y-2 shrink-0">
        ...
        {checkInCount >= 10 ? (
          <div className="flex justify-center">
            <button
              ...
              style={{ minHeight: '40px' }}
            >
              <span>{language === 'en' ? '🧺 Harvest Garden (+10 pts)' : '🧺 收割成果（+10積分）'}</span>
            </button>
          </div>
        ) : (
          <div className="flex justify-center">
            ...
            >
              <span>
                {language === 'en' 
                  ? `🧺 Harvest (${displayCount}/10)` 
                  : `🧺 收割成果（未達到收割條件 ${displayCount}/10）`
                }
              </span>
            </button>
          </div>
        ...
        <div className="flex justify-center pt-1">
          <button
            ...
            style={{ minHeight: '40px' }}
          >
            <span>{language === 'en' ? '🎁 Visit Garden Shop' : '🎁 前往「花園商店」'}</span>
          </button>
        </div>
      </div>
      ...
      <div className="bg-[#f9f7f2] p-3.5 rounded-2xl border border-brand-sand/60 space-y-2 shrink-0">
        <span className="text-[14px] sm:text-[15px] font-black text-brand-moss block">
          {language === 'en' ? '💡 How to Earn Points:' : '💡 獲取積分方法：'}
        </span>
        <ul className="text-[13px] sm:text-[14px] font-bold text-gray-500 space-y-1.5 font-sans leading-relaxed">
          <li className="flex items-center gap-1.5">
            <span className="text-emerald-500">✅</span>
            <span>{language === 'en' ? 'Daily mood check-in' : '每日記錄心情'} <span className="text-brand-sage font-black">+1</span></span>
          </li>
          <li className="flex items-center gap-1.5">
            <span className="text-emerald-500">✅</span>
            <span>{language === 'en' ? 'Daily plant watering' : '每日澆花'} <span className="text-brand-sage font-black">+1</span></span>
          </li>
          <li className="flex items-center gap-1.5">
            <span className="text-emerald-500">✅</span>
            <span>{language === 'en' ? 'Write diary entry (>10 words)' : '寫日誌超過 10 字'} <span className="text-brand-sage font-black">+1</span></span>
          </li>
          <li className="flex items-center gap-1.5">
            <span className="text-emerald-500">✅</span>
            <span>{language === 'en' ? '3-day check-in streak' : '連續 3 日記錄心情'} <span className="text-brand-ochre font-black">+2</span></span>
          </li>
          <li className="flex items-center gap-1.5">
            <span className="text-emerald-500">✅</span>
            <span>{language === 'en' ? 'Daily quote card claim' : '每日領取語錄卡'} <span className="text-brand-sage font-black">+1</span></span>
          </li>
          <li className="flex items-center gap-1.5">
            <span className="text-emerald-500">✅</span>
            <span>{language === 'en' ? 'Complete First Aid exercise' : '每日完成休息站練習'} <span className="text-brand-sage font-black">+1</span></span>
          </li>
          <li className="flex items-center gap-1.5">
            <span className="text-emerald-500">✅</span>
            <span>{language === 'en' ? 'Harvest garden' : '收割成果'} <span className="text-brand-sage font-black">+10</span></span>
          </li>
        </ul>
      </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="space-y-3">
                <h3 className="text-lg font-black text-brand-moss">
                  {language === 'en' ? 'Confirm Harvest?' : '確定收割？'}
                </h3>
                ...
                <div className="bg-[#fcfaf5] p-4 rounded-2xl border border-brand-sand/60 space-y-2 text-center text-sm font-bold text-gray-700 leading-relaxed shadow-xs">
                  <p className="text-brand-moss font-black text-[15px]">
                    {language === 'en' ? 'After harvesting, your plant will reset with a new seed.' : '收割後，盆栽將迎來新的種子'}
                  </p>
                  <p className="text-gray-600 font-extrabold text-sm">
                    {language === 'en' ? 'Ready to accompany your new plant again.' : '等待你再次陪伴它成長'}
                  </p>
                  <div className="pt-2 text-brand-ochre font-black text-base flex items-center justify-center gap-1 border-t border-brand-sand/40 mt-1">
                    <span>{language === 'en' ? '✨ Earn' : '✨ 可獲得'}</span>
                    <span className="text-emerald-600 text-lg font-black">+10 {language === 'en' ? 'pts' : '積分'}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center justify-center gap-3 pt-2">
                ...
                >
                  <span>❌ {language === 'en' ? 'Cancel' : '取消'}</span>
                </button>
                <button
                  onClick={() => {
                ...
                >
                  <span>✅ {language === 'en' ? 'Confirm' : '確定'}</span>
                </button>
              </div>
            </motion.div>
          </div>
